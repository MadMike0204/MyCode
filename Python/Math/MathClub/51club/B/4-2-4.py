...
def process_all_combinations_parallel(all_combinations, original_lines, boundary_polygon, coordinates, batch_size=1000):
    ...
    with Pool() as pool:
        for i in range(0, len(combo_args), batch_size):
            ...
            save_combinations_to_excel(valid_results, coordinates, i // batch_size)
...
def save_combinations_to_excel(combination_batch, coordinates, batch_index):
    file_path = r'F:\------------------OWN\VSCode\Python\Math\MathClub\51club\B\comb2\combinations_batch_{}.xlsx'.format(batch_index)
...
def main():
    ...
    process_all_combinations_parallel(all_possible_combinations, lines, boundary_polygon, coordinates)
