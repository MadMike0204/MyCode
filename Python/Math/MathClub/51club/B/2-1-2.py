...
def main():
    ...
    with pd.ExcelWriter('GP-Problem2-data'+datetime.now().strftime("%m%d%H%M")+'.xlsx') as writer:
        for gen in range(NGEN):
            ...
            for fit, ind in zip(fits, offspring):
                ...
                for (source, target), paths in paths_dict.items():
                    ...
                    if (source, target) == (27,6) and gen >= (NGEN-5):
                        for path_idx, path in enumerate(paths):
                            path_demand = demands[(source, target)] * normalized_weights[path_idx]
                            path_info_1.append({'Source': (source,target), 'Path': path, 'Assigned Demand': path_demand})

                    if (source, target) == (19,25) and gen >= (NGEN-5):
                        for path_idx, path in enumerate(paths):
                            path_demand = demands[(source, target)] * normalized_weights[path_idx]
                            path_info_2.append({'Source': (source,target), 'Path': path, 'Assigned Demand': path_demand})

            df_path_info = pd.DataFrame(path_info_1)
            df_path_info.to_excel(writer, sheet_name=f'27-6-Generation_{gen}_', index=False)
            df_path_info = pd.DataFrame(path_info_2)
            df_path_info.to_excel(writer, sheet_name=f'19-25-Generation_{gen}', index=False)
            #进化
