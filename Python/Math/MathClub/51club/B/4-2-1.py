...
def generate_six_combinations(valid_combinations):
    return list(combinations(valid_combinations, 6))

def save_combinations_to_files(six_combinations):
    for idx, combination in enumerate(six_combinations):
        filename = fr'F:\------------------OWN\VSCode\Python\Math\MathClub\51club\B\comb\combination_{idx}.json'
...
#点坐标信息
    ...
    # 生成所有6路段组合
    six_combinations = generate_six_combinations(valid_combinations)

    # 保存到文件
    save_combinations_to_files(six_combinations)
