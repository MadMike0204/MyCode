...
int LinkList::Swap2Node(int a,int b)
{
    ...
    GetElement(a-1)->next = GetElement(b);
    GetElement(b-1)->next = GetElement(a);
    GetElement(a)->next = tempBnext;
    GetElement(b)->next = tempAnext;
    return 1;
}
...
int main(){
    ...
    cin >> idx;
    if(jo.FindElement(idx))
    {
        cout << jo.FindElement(idx) << endl;
    }
    else
    {
        cout << "error" << endl;
