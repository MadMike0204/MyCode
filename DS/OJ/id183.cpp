...
class BinaryTree
{
    ...
    private:
        ...
        int GetNumberOfLeafNode(BinaryTreeNode* t);
    public:
        BinaryTree();// 置空树
...
// ---
int BinaryTree::GetNumberOfLeafNode(BinaryTreeNode* t)
{
    if(!t) // 节点存在
        return 0; 
    if(t->GetLeftChild() || t->GetRightChild()) // 节点存在下级节点
        return GetNumberOfLeafNode(t->GetLeftChild()) + GetNumberOfLeafNode(t->GetRightChild());
    return 1; // 剩下的情况即节点不存在下级节点
}
int BinaryTree::GetNumberOfLeafNode()
{
    return GetNumberOfLeafNode(this->root);
}
// ---
int main(){
    ...
    while(t--)
    {
        ...
        cout << tree.GetNumberOfLeafNode() << endl;
    }
