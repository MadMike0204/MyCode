...
int Maze2D::FindConsecutivePassage(Pos currentPos) {
    // 查找所有可能的通道，依次检查四个方向
    if (currentPos.y + 1 < size && maze[currentPos.x][currentPos.y + 1] == 0) // 向右
        return 0;
    if (currentPos.x + 1 < size && maze[currentPos.x + 1][currentPos.y] == 0) // 向下
        return 1;
    if (currentPos.y - 1 >= 0 && maze[currentPos.x][currentPos.y - 1] == 0)   // 向左
        return 2;
    if (currentPos.x - 1 >= 0 && maze[currentPos.x - 1][currentPos.y] == 0)   // 向上
        return 3;
...
Pos Maze2D::CurrentPositionAndNextDirectionToPosition(Pos currentPos, int direction) {
    ...
    nextPos.direction = -1;
    switch (direction) {
    case 0: nextPos.y++; break;  // 向右走
    case 1: nextPos.x++; break;  // 向下走
    case 2: nextPos.y--; break;  // 向左走
    case 3: nextPos.x--; break;  // 向上走
    }
    return nextPos;
}
