...
void LinkList::ListDisplayNoBlankEnding(){
    LNode* p = head->next;
    while(p != nullptr)
    {
        cout << p->data;
...
int main(){
    ...
    int t;
    cin >> t;
    int rt,value,location;
    LinkList jojo;

    while(t--){
        cin >> location >> value;
        rt = jojo.ListInsert(location,value);
        if(!rt){
            cout << "error" << endl;
            continue;
        }
        jojo.ListDisplayNoBlankEnding();
        cout << endl;
    }
