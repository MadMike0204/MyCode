...
class CStudentID
{
    ...
    public:
        CStudentID(string &p_id_in,char* p_name_in,CDate &day)
        {   
            int year_temp,month_temp,day_temp;
            int length_id = p_id_in.length();
            int length_name = strlen(p_name_in);
            p_name = new char[length_name+1];
            for(int i=0;i<length_name;i++)
            {
                p_name[i] = p_name_in[i];
            }
